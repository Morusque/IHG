...
boolean dark = false;
float darkFadeLevel = 0.0;
float darkFadeTarget = 0.0;
...
void draw() {
  ...
  if (dark) {
    darkFadeTarget = 1.0;
    darkFadeLevel += constrain(darkFadeTarget-darkFadeLevel, -0.05, 0.05);
  } else {
    darkFadeTarget = 0.0;
    darkFadeLevel += constrain(darkFadeTarget-darkFadeLevel, -0.05, 0.05);
  }
  if (darkFadeLevel>0.0) {
    noStroke();
    fill(0, darkFadeLevel*0xFF);
...
void keyPressed() {
  ...
  if (key=='i') {
    // add a sprite somewhere (2)
    synchronized(spritesIm) {
      sprites.add(new Sprite(spritesIm.get(2)));
    }
  }
