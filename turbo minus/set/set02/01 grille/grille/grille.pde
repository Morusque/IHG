...
boolean transitionToBlack = false;
...
void setup() {
  ...
  decks = sort(decks);
  loadDataFiles(decks[currentDeckIndex]);
  pattern = new Pattern();
  ...
  controlWindow = new PWindow(this);
}
...
void draw() {
  ...
  if (transitionToBlack) {
    for (int i=0; i<2000; i++) {
      int iX = floor(random(colorIndexes.length));
      int iY = floor(random(colorIndexes[0].length));
      colorIndexes[iX][iY]=-1;
    }
    boolean blackYet=true;
    for (int x=0; x<colorIndexes.length; x++) {
      for (int y=0; y<colorIndexes[0].length; y++) {
        if (colorIndexes[x][y]!=-1) blackYet=false;
      }
    }
    if (blackYet) transitionToBlack = false;
  }
  pattern.update();
  nextPattern.update();
...
void loadDataFiles(String currentDataFolder) {
  println(currentDataFolder);
  facesSubfolders = getSubfolders(dataPath(currentDataFolder+"/faces"));
  facesSubfolders = sort(facesSubfolders);
  currentSubfolderIndex = 0;
  loadFacesImgFiles(currentDataFolder);
  String[] messagesFiles = getAllFilesFrom(dataPath(currentDataFolder+"/messages"));
...
void loadFacesImgFiles(String currentDataFolder) {
  String[] facesImgFiles = getAllFilesFrom(dataPath(currentDataFolder+"/faces/"+facesSubfolders[currentSubfolderIndex]));
  facesIms.clear();
...
void keyPressed() {
  ...
  if (key=='q') {
    loadFacesImgFiles(decks[currentDeckIndex]);
  }
  if (keyCode=='1') {
    ...
    for (int x=0; x<colorIndexes.length; x++) {
      for (int y=0; y<colorIndexes[x].length; y++) {
        colorIndexes[x][y]=colorIndexes[x][y]%min(typicalColors.length, facesIms.size());
      }
    }
    controlWindow.logTxt("loaded deck : "+decks[currentDeckIndex]);
...
class PWindow extends PApplet {
  boolean useControlWindow = true;
  ArrayList<String> lines = new ArrayList<String>();
  grille parent;

  PWindow(grille parent) {
    this.parent = parent;
    if (useControlWindow) {
  ...
  public void draw() {
    ...
    for (int i=0; i<lines.size(); i++) {
      text(lines.get(i), 50, 50+i*20);
    }
  }

  public void logTxt(String text) {
    lines.add(text);
    while (lines.size()>15) lines.remove(0);
  }
}
