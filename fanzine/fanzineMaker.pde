...
/*
FanzineMaker fanzineMaker = new FanzineMaker(0);
...
class FanzineMaker {
  ...
  void order() {
    println("ordering...");
    orderedPages.clear();
    orderedPages = shuffle(images);
    orderedPages.set(0, first);
    PGraphics blank = createGraphics(50, 50, JAVA2D);
    blank.beginDraw();
    blank.background(0xFF);
    blank.endDraw();
    while (orderedPages.size() < nbPages-1) orderedPages.add(blank.get());
    while (orderedPages.size() >= nbPages) orderedPages.remove(orderedPages.size()-1);
    orderedPages.add(last);
    println("... done");
  }
