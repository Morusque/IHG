...
class CustomShape {
  ...
  void update() {
    ...
    if (dist(0, 0, body.getLinearVelocity().x, body.getLinearVelocity().y) < 0.01 && body.getType()==BodyType.DYNAMIC) {
      // body.setType(BodyType.STATIC);
      Vec2 thisPos = box2d.getBodyPixelCoord(body);
      int targetPlate = floor(thisPos.x*nbVegs.length/width);
      for (int i=0; i<nbVegs.length; i++) {
        if (i==targetPlate) polygons.get(i).move(0, -10);
        else polygons.get(i).move(0, +10);
      }
    }
    if (body.getType()==BodyType.KINEMATIC) {
      body.setLinearVelocity(new Vec2(body.getLinearVelocity().x*0.8f, body.getLinearVelocity().y*0.8f));
    }
    life+=1;
  }
